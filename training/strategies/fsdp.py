...
from training.strategies.base_strategy_geovla import TrainingStrategy
...
class FSDPStrategy(TrainingStrategy):
    ...
    def save_checkpoint(
        ...
        with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy, self.fsdp_save_optimizer_policy):
            ...
            if overwatch.is_rank_zero():
                ...
                overwatch.info(f"Saved model weight to {checkpoint_path}")
            dist.barrier()
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        if self.lr_scheduler_type == "linear-warmup+cosine-decay":
            ...
            # Default AdamW w/ specified LR & Linear Warmup / Cosine Decay & Weight Decay
            #   => Create Parameter Groups --> bias terms, normalization layer parameters shouldn't be decayed!
            decay, no_decay, new_module = [], [], []
            for name, param in self.vlm.named_parameters():
                if not param.requires_grad:
                    continue

                ########### add for new added modules ###############
                new_module_list = ["states_encoder", "states_decoder", "depth_encoder", 
                                   "goal_encoder", "pcd_cond_fusion", "img_lang_fusion"]
                if any(module in name for module in new_module_list):
                    if overwatch.is_rank_zero():
                        print(f"{name} is in new_module trained with 10x LR")
                    new_module.append(param)
                    continue
                ########### add for new added modules ###############
            ...
            # Build Parameter Groups
            groups = [{"params": decay, "weight_decay": self.weight_decay, "lr": self.learning_rate}, 
