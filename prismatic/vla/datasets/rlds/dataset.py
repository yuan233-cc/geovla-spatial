...
def make_dataset_from_rlds(
    ...
    *,
    ...
    if action_normalization_mask is not None:
        ...
        # proprio should not be normalized, modified by sunl
        if "proprio" in dataset_statistics:
            # dataset_statistics["proprio"]["mask"] = np.array([True] * dataset_statistics['proprio']['min'].shape[0]) # all normalize for maniskill
            dataset_statistics["proprio"]["mask"] = np.array([False] * dataset_statistics['proprio']['min'].shape[0]) # no normalize for real and libero
...
def make_interleaved_dataset(
    ...
    *,
    ...
    # merge all the actions and pro to whole
    action_q01, action_q99, proprio_q01, proprio_q99 = None, None, None, None
    for name, statics in all_dataset_statistics.items():
        s_q01, p_q01 = statics['action']['q01'], statics['proprio']['q01']
        s_q99, p_q99 = statics['action']['q99'], statics['proprio']['q99']
        action_q01 = s_q01 if action_q01 is None else np.where(action_q01 < s_q01, action_q01, s_q01)
        action_q99 = s_q99 if action_q99 is None else np.where(action_q99 > s_q99, action_q99, s_q99)
        
        # proprio_q01 = p_q01 if proprio_q01 is None else np.where(proprio_q01 < p_q01, proprio_q01, p_q01)
        # proprio_q99 = p_q99 if proprio_q99 is None else np.where(proprio_q99 > p_q99, proprio_q99, p_q99)
    
    for name in all_dataset_statistics.keys():
        all_dataset_statistics[name]['action']['q01'] = action_q01
        all_dataset_statistics[name]['action']['q99'] = action_q99
        # all_dataset_statistics[name]['proprio']['q01'] = proprio_q01
        # all_dataset_statistics[name]['proprio']['q99'] = proprio_q99
