...
class PrismaticVLM(VLM):
    def __init__(
        ...
        **kwargs,
        ...
        # add vision_fusion
        from modules import VisionFusionModule
        self.vision_fusion = VisionFusionModule(input_embedding_dim=vision_backbone.embed_dim, embedding_dim=llm_backbone.embed_dim, num_attn_heads=8, num_layers=2)

        # variables for geovla
        self.img_patches = 256
        self.img_embeddings = None # [B, N, C]
        self.lang_embedings = None # [B, N, C]
        #

        # Set Module Keys =>> used in Checkpoint Saving / Model Loading
        self.all_module_keys = ["vision_backbone", "llm_backbone", "projector", "vision_fusion"]
        self.trainable_module_keys = ["vision_fusion"]
    ...
    def forward(
        ...
        query_pos, value_pos = others.pop("query_pos", None), others.pop("value_pos", None)
        with torch.set_grad_enabled(self.vision_backbone_requires_grad):
            if isinstance(pixel_values, dict):
                patch_features = None
                for k, v in pixel_values.items():
                    if list(v.values())[0].sum() == 0: continue # all is zero --> moe type is pcd
                    patch_feature = self.vision_backbone({k: v[k][multimodal_indices] for k in v}, **input_kwargs)
                    patch_features = self.vision_fusion(
                        main_embeddings=patch_features, other_embeddings=patch_feature,
                        main_pos=query_pos, other_pos=value_pos,
                    ) if patch_features is not None else self.projector(patch_feature)
            else:
                patch_features = self.vision_backbone(pixel_values[multimodal_indices], **input_kwargs)
                patch_features = self.projector(patch_features)

        # Projection Logic :: [bsz, num_patches, llm_embed_dim] =>> num_patches = (2 *) (256 + 1) for ViT-L + CLS
        patch_features = patch_features if patch_features is not None else torch.zeros((query_pos.shape[1], 0, 4096)).to(query_pos)
        self.img_patches = patch_features.shape[1]
        projected_patch_embeddings = patch_features
        ...
        # set embeddings for geovla
        self.img_embeddings = projected_patch_embeddings
        self.lang_embeddings = input_embeddings
        
        others_attention_mid_mask, others_attention_last_mask = {}, {}
        others_mid, others_last = {}, {}

        if attention_mask is not None:
                    
            for k, v in others.items():
                if "late" in k:
                    others_attention_last_mask[k] = torch.full(
                        (v.shape[0], v.shape[1]), True,
                        dtype=attention_mask.dtype, device=attention_mask.device,
                    )
                    others_last[k] = v
                else:
                    others_attention_mid_mask[k] = torch.full(
                        (v.shape[0], v.shape[1]), True,
                        dtype=attention_mask.dtype, device=attention_mask.device,
                    )
                    others_mid[k] = v
        ...
        multimodal_embeddings = torch.cat(
            [
                ...
                *list(others_mid.values()),
                input_embeddings[multimodal_indices, 1:-1, :],
                *list(others_last.values()),
                input_embeddings[multimodal_indices, -1:, :],
            ],
            dim=1,
        )
        ...
        if attention_mask is not None:
            multimodal_attention_mask = torch.cat(
                [
                    ...
                    *list(others_attention_mid_mask.values()),
                    attention_mask[multimodal_indices, 1:-1],
                    *list(others_attention_last_mask.values()),
                    attention_mask[multimodal_indices, -1:],
                ],
                dim=1,
            )
        ...
        if labels is not None:
            ...
            others_mid_labels, others_last_labels = {}, {}
            for k, v in others.items():
                if "late" in k:
                    others_last_labels[k] =  torch.full(
                        (v.shape[0], v.shape[1]), IGNORE_INDEX,
                        dtype=labels.dtype, device=labels.device,
                    )
                else:
                    others_mid_labels[k] =  torch.full(
                        (v.shape[0], v.shape[1]), IGNORE_INDEX,
                        dtype=labels.dtype, device=labels.device,
                    )
            ...
            multimodal_labels = torch.cat([
                ...
                projected_patch_labels, *list(others_mid_labels.values()),
                labels[multimodal_indices, 1:-1], *list(others_last_labels.values()),
                labels[multimodal_indices, -1:],
                ], dim=1
            )
