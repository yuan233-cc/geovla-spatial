...
if [[ " $@ " =~ " --debug " ]]; then
    NUMS=(1 1 1 1) # (8 8 256 32) (1 1 4 4)
    DEBUG=True
    SAVE_PATH=debug
...
# x4_3d_base_task, x14_shift_task, x4_3d_black_task, x4_3d_task
## widowx ###################################################################################################
# SAVE_PATH=20250704_widowx_3dvla_x4_3d_task
...
#   --data_root_dir /mnt/sunl-benchmark/datasets/widowx_4/tfds/delta_auto_05cm_10ang_drop \
...
# SAVE_PATH=20250706_maniskill_3dvlamoe_2ori_10000
# OUTPUT_DIR=/data/outputs/geovla/maniskill

# torchrun --standalone --nnodes 1 --nproc-per-node ${NUMS[0]} scripts/train.py \
#   --pretrained_checkpoint /data/model/hub/openvla-7b-prismatic/checkpoints/step-295000-epoch-40-loss=0.2200.pt \
#   --vla.type prism-dinosiglip-224px+oxe+diffusion \
#   --vla.data_mix "agg_maniskill_with_state_wrist_pc_tcp" \
#   --vla.expected_world_size ${NUMS[1]} \
#   --vla.global_batch_size ${NUMS[2]} \
#   --vla.per_device_batch_size ${NUMS[3]} \
#   --vla.learning_rate 2e-5 \
...
##################################################################################################################
# libero_all_with_state_wrist_pc, libero_100_state_pc_no_noop, libero_fun_state_pc_no_noop
SAVE_PATH=20250715_libero_fun_3dvlamoe_from28
OUTPUT_DIR=/mnt/sunl-benchmark/outputs/geovla/libero

torchrun --standalone --nnodes 1 --nproc-per-node ${NUMS[0]} scripts/train.py \
  --pretrained_checkpoint /mnt/sunl-benchmark/outputs/geovla/libero/20250711_libero_all_3dvlamoe_100000/checkpoints/step-028000-epoch-08-loss=0.0222.pt \
  --vla.type prism-dinosiglip-224px+oxe+diffusion \
  --vla.data_mix "libero_fun_state_pc_no_noop" \
  --vla.expected_world_size ${NUMS[1]} \
  --vla.global_batch_size ${NUMS[2]} \
  --vla.per_device_batch_size ${NUMS[3]} \
  --vla.learning_rate 2e-5 \
