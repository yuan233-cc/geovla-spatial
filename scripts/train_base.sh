...
## widowx ###################################################################################################
# SAVE_PATH=20250716_widowx_geovla_x4_higher_task
...
#   --data_root_dir /mnt/sunl-benchmark/datasets/widowx_4/tfds/delta_auto_05cm_10ang_drop_camera \
...
# maniskill ####################################################################################################
SAVE_PATH=20250702_maniskill_base_new_ori_shuff10000
OUTPUT_DIR=/data/outputs/geovla/maniskill

torchrun --standalone --nnodes 1 --nproc-per-node ${NUMS[0]} scripts/train.py \
  --pretrained_checkpoint /data/model/hub/openvla-7b-prismatic/checkpoints/step-295000-epoch-40-loss=0.2200.pt \
  --vla.type prism-dinosiglip-224px+oxe+diffusion \
  --vla.data_mix "pick_cube_pcd_goal_state" \
  --vla.expected_world_size ${NUMS[1]} \
  --vla.global_batch_size ${NUMS[2]} \
  --vla.per_device_batch_size ${NUMS[3]} \
  --vla.learning_rate 2e-5 \
  ...
  --load_depth False \
  --depth_type none \
  --proprio_type goal+vlm_condition@8 \
  --ssh_debug $DEBUG \
  --load_wrist False \
  --wrist_first False \
...
# SAVE_PATH=20250712_libero_all_base_noop_100000
